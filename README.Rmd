...
__bayesRates__ is an R package that allows users to (1) perform two-sample tests with binomial and Poisson data from a Bayesian perspective, (2) view decision rules to understand them better, and (3) determine sample sizes for designing such procedures.  The package currently works for binomial and Poisson data.
...
This output is _long_!  Why?  What is going on here?  Given the prior specifications, one can write down a formula for the 
